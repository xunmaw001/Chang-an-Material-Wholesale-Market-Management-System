            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑商户</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 shanghuNameDiv">
                                        <label>商户姓名</label>
                                    ...
                                    <div class="form-group col-md-6 shanghuIdNumberDiv">
                                        <label>身份证号</label>
                                        <input id="shanghuIdNumber" name="shanghuIdNumber" class="form-control"
                                               onchange="shanghuIdNumberChickValue(this)"  placeholder="身份证号">
                                    </div>
                                    <div class="form-group col-md-6 shanghuPhoneDiv">
                                        <label>手机号</label>
                                        <input id="shanghuPhone" name="shanghuPhone" class="form-control"
                                               onchange="shanghuPhoneChickValue(this)"  placeholder="手机号">
                                    </div>
                                    <div class="form-group col-md-6 shanghuEmailDiv">
                                        <label>邮箱</label>
                                        <input id="shanghuEmail" name="shanghuEmail" class="form-control"
                                               placeholder="邮箱">
                                    </div>
                                    <div class="form-group col-md-6 shanghuPhotoDiv">
                                        <label>照片</label>
                                        <img id="shanghuPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="shanghuPhotoupload"
                                               class="form-control-file">
                                        <input name="shanghuPhoto" id="shanghuPhoto-input" type="hidden">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#shanghuPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('shanghuPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('shanghuPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("shanghu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addshanghu');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function shanghuIdNumberChickValue(e){
        ...
        function shanghuPhoneChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                sexTypes: "required",
                shanghuIdNumber: "required",
                shanghuPhone: "required",
                shanghuEmail: "required",
                shanghuPhoto: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                shanghuName: "商户姓名不能为空",
                sexTypes: "性别不能为空",
                shanghuIdNumber: "身份证号不能为空",
                shanghuPhone: "手机号不能为空",
                shanghuEmail: "邮箱不能为空",
                shanghuPhoto: "照片不能为空",
            }
        }).form();
    ...
    // 获取当前详情
    function getDetails() {
        var addshanghu = window.sessionStorage.getItem("addshanghu");
        if (addshanghu != null && addshanghu != "" && addshanghu != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("shanghu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
